...
#     def aggregate(self, reports, collection, output_path, verbose):
    def aggregate(self, collection):
#     def aggregate(self, documents):
        labels = []
...
#                     labeled_reports[category] = temp_labels[:, index]
#                 temp_output_path = output_path.split(".")[0]+"_" + str(idx) + ".csv"
#                 if verbose:
#                     print(f"Writing reports and labels to {temp_output_path}.")
#                 labeled_reports[[REPORTS] + CATEGORIES].to_csv(temp_output_path,
#                                                                index=False)
